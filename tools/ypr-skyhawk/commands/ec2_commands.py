...
@click.option('--wait', is_flag=True, default=False, help='Wait until instance is fully stopped')
...
@click.option('--wait', is_flag=True, default=False, help='Wait until instance is fully running')
@click.option('--region', default='us-east-1', help='AWS region name')
...
@click.option('--output', type=click.Choice(['table', 'json']), default='table', help='Output format: table or json')
@click.option('--region', default='us-east-1', help='AWS region name')
@click.option('--save-path', required=False, help='File path to save output (CSV or JSON based on format)')
def list_instances(state: str = None, output: str = 'table', region: str = 'us-east-1', save_path: str = None):
    """
    ...
    if output == 'json':
        ...
        headers = ['InstanceId', 'State', 'Type', 'Public IP', 'Private IP', 'Launch Time']
        ...
        if output == 'json':
            ...
            with open(save_path, 'w', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=[
                    'InstanceId', 'State', 'InstanceType', 'PublicIpAddress', 'PrivateIpAddress', 'LaunchTime'
                ])
                writer.writeheader()
