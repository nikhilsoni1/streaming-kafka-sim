...
class ChartAccelRawXYZ(Chart):
    ...
    def generate(self, log_data: dict):
        ...
        fig.add_trace(
            go.Scatter(x=df["timestamp"], y=df["accelerometer_m_s2[0]"],
                       mode="lines", name="Accel X"),
            row=1, col=1
        )
        fig.add_trace(
            go.Scatter(x=df["timestamp"], y=df["accelerometer_m_s2[1]"],
                       mode="lines", name="Accel Y"),
            row=2, col=1
        )
        fig.add_trace(
            go.Scatter(x=df["timestamp"], y=df["accelerometer_m_s2[2]"],
